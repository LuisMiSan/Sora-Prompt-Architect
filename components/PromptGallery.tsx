...
const VersionHistoryModal: React.FC<{
  ...
  return (
    ...
    >
      <div 
        className="bg-brand-surface w-full max-w-3xl max-h-[80vh] rounded-2xl shadow-xl flex flex-col border border-brand-border"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex justify-between items-center p-5 border-b border-brand-border">
          <h3 className="text-xl font-bold text-brand-text-primary">{t('gallery.versionHistoryTitle')}</h3>
          ...
          {prompt.versions.map((version, index) => (
            <div key={version.createdAt} className="bg-slate-50 p-4 rounded-lg border border-brand-border">
              <div className="flex justify-between items-start mb-3 gap-4">
                <div>
                  <p className="text-sm text-brand-text-secondary">
                    {t('gallery.versionCreatedOn')} <span className="font-semibold text-brand-text-primary">{new Date(version.createdAt).toLocaleString()}</span>
                    {index === 0 && <span className="ml-2 text-xs font-bold text-cyan-800 bg-cyan-100 px-2 py-0.5 rounded-full">{t('gallery.latest')}</span>}
                  </p>
                </div>
                <button 
                  onClick={() => { onRemix(prompt.id, version); onClose(); }}
                  className="bg-brand-accent-to hover:bg-brand-accent-hover text-white font-semibold text-xs py-1.5 px-3 rounded-lg transition-colors whitespace-nowrap flex items-center justify-center gap-1.5"
                >
              ...
              {version.versionNotes && (
                <div className="mb-3">
                  <p className="text-xs text-brand-text-secondary font-semibold">{t('gallery.versionNotesLabel')}</p>
                  <p className="text-sm text-brand-text-primary italic">"{version.versionNotes}"</p>
                </div>
              )}
              <div className="bg-brand-surface p-3 rounded-lg max-h-40 overflow-y-auto border border-brand-border">
                 <p className="text-brand-text-primary text-sm whitespace-pre-wrap">{version.prompt}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  if (prompts.length === 0) {
    return (
      <div className="text-center py-16">
        <h2 className="text-3xl font-bold text-brand-text-primary mb-2">{t('gallery.empty.title')}</h2>
        <p className="text-brand-text-secondary max-w-md mx-auto">{t('gallery.empty.subtitle')}</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="space-y-8 animate-fade-in">
       <div className="text-center">
         <h2 className="text-4xl font-bold text-brand-text-primary">{t('gallery.title')}</h2>
         <p className="text-brand-text-secondary mt-2">{t('gallery.subtitle')}</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {prompts.map(prompt => {
          ...
          return (
          <div key={prompt.id} className="group bg-brand-surface p-5 rounded-2xl border border-brand-border flex flex-col justify-between transition-all duration-300 hover:scale-[1.03] hover:shadow-glow hover:border-brand-accent-to/50 shadow-lg">
            <div className="flex flex-col space-y-4">
                 <div className="flex justify-between items-start">
                    <p className="text-brand-text-primary line-clamp-6 font-medium pr-4 text-sm">{latestVersion.prompt}</p>
            ...
            <div className="flex items-center justify-between gap-2 mt-auto pt-4 border-t border-brand-border mt-4">
                <p className="text-xs text-brand-text-secondary">{new Date(firstVersion.createdAt).toLocaleDateString()}</p>
                <div className="flex gap-2">
                    ...
                    className="p-2 bg-brand-accent-to hover:bg-brand-accent-hover rounded-lg transition-colors"
                    title={t('gallery.remixScene')}
                    >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                    </button>
                    <button
